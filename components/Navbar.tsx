...
const Navbar: React.FC<NavbarProps> = ({ isScrolled }) => {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          <div className="flex-shrink-0 flex items-center gap-2 sm:gap-3">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-teal-500 to-teal-700 rounded-2xl flex items-center justify-center shadow-lg">
            ...
            <span className={`text-lg sm:text-2xl font-extrabold tracking-tight ${isScrolled ? 'text-teal-700' : 'text-teal-600'}`}>Teeth Care</span>
          </div>

          <div className="hidden md:flex space-x-6 md:space-x-8 items-center">
            {menuItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
            ...
            <a
              href="#contact"
              className="btn shadow-md px-5 md:px-7 py-2 text-base font-semibold ml-2"
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-slate-100 px-3 py-5 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300 rounded-b-3xl">
          <div className="flex flex-col space-y-3">
            {menuItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-base text-slate-700 font-semibold hover:text-teal-600 border-b border-slate-50 pb-2 px-2 rounded-lg transition-colors"
            ...
            <a
              href="#contact"
              className="btn text-center py-3 font-bold shadow-lg"
              onClick={() => setIsMobileMenuOpen(false)}
            >
